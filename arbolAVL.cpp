...
int calcHeight(NodeAVL *&t)
   {
   		if(t==nullptr)
   			return -1;
   		if(balanceo(t)=='/' || balanceo(t)=='-')
...
int main()
{
    ...
    int cantidad;   
    cout<<"cantidad de NodeAVLs ? ";
    cin>>cantidad;
    int dato;
    int altura;
    for(int i=0;i<cantidad;i++)
    {
        cout<<"ingrese un NodeAVL ";
        cin>>dato;
        insertar(dato,head);
    }   
    cout<<"mostrando arbol "<<endl;
    verArbol(head,0);
    cout<<"altura del arbol balanceado "<<endl;
    cout<<AlturaArbol(head,&altura);
    return 0;
}